...
- **2026-09-18 초기 구축**: 포크 → 리브랜딩(RoadQ·EX 블루 `#00539F`·스토리지 `roadq.*`) → `expressway` 팩 신설(시나리오 2·지도 8본부·라이브 지표·지식그래프) → **데이터 탭(시계열 인텔리전스) 신규 개발** → 코어 기본값·에이전트 13종·관리자 61메뉴 도로공사 전환 → 문서·하네스 승계 체계 정비.
- 잡은 결함: 팩 `docs[]`에 `tags`·`secLevel` 누락 → 우측 패널 크래시(화이트 스크린), 예측 곡선의 급락 시점이 '현재'와 어긋나 관측값이 원장(38km/h)과 불일치.
- 다음 후보: **[docs/ROADMAP.md](docs/ROADMAP.md)** 참조.
